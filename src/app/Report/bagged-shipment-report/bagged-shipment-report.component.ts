...
@Component({
  selector: 'app-bagged-shipment-report',
  templateUrl: './bagged-shipment-report.component.html',
  styleUrls: ['./bagged-shipment-report.component.css']
})
export class BaggedShipmentReportComponent implements OnInit {
...
fetchCheckWeightData:any[]=[]
  ...
  getreport(){
    this.serviceNew.v1_SP_Report_carton_bagged(this.valForm.value).subscribe(d=>{
...
callApi(searchText: string) {
  ...
  this.serviceNew.v1_SP_Report_carton_bagged(payload).subscribe(
    data => {
      console.log(data);
      ...
      if (newInvoices && newInvoices.length > 0) {
        ...
        const filteredInvoices = newInvoices.filter(
          (invoice) => !this.fetchCheckWeightData.some(
            (existingInvoice) => existingInvoice.merged_hawb === invoice.merged_hawb
          )
        );
