...
@Component({
  selector: 'app-packing-list-report',
  templateUrl: './packing-list-report.component.html',
  styleUrls: ['./packing-list-report.component.css']
})
export class PackingListReportComponent implements OnInit {
 @ViewChild('mainGrid') mainGrid:GridComponent;
valForm:FormGroup;
userDetails:any
fetchCheckWeightData:any[]=[]
  constructor(public _chat: ChatService , private datep: DatePipe, fb: FormBuilder,
    public service: AwbService,  
  ...
  getreport(){
    this.serviceNew.v1_sp_report_packinglist_details_get(this.valForm.value).subscribe(d=>{
...
formateUsingChatGPt() {
  ...
  if (data.length === 0) {
    ...
    this.showMessage('warning','All STS is already updated or no packages to update')
...
ReformateUsingChatGPt() {
  ...
  // const data = chatgot.filter(d => d['chatgpt_updated_status'] != 1);
  console.log(this.mainGrid.getSelectedRecords())
  if (this.mainGrid.getSelectedRecords().length === 0) {
    // this.toastr.warningToastr("All STS is already updated or no packages to update");
    this.showMessage('warning','All STS is already updated or no packages to update')
 
    return;
  }
const fullData= this.mainGrid.getSelectedRecords().map(r=>({
  ...r,
      inv_no: r['hawb_no'],
    prompt: r['item_details'],
    carton_id: r['carton_id'],
