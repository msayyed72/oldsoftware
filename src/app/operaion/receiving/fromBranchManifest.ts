...
@Component({
    moduleId: module.id,
    templateUrl: './fromBranchManifest.html',
})
export class FromBranchManifestComponent implements OnInit {
      ...
      getNonClearedCartonDetailsReceivingBranch(){
        ...
        this.service.getNonClearedCartonDetailsReceivingBranch(this.manifest,this.parent_manifest).subscribe(get_data=>{
         ...
         for (let i = 0; i < this.NonClearedCartonDetailsReceivingBranch.length; i++) {
            ...
            // console.log("aa", this.NonClearedCartonDetailsReceivingBranch[i]['carton_no']);  
             this['tot_non_clr_wgt'] = Number(this.NonClearedCartonDetailsReceivingBranch[i]['weight'])+Number(this['tot_non_clr_wgt']) 
            this['tot_non_clr_ctn'] = 1*this.NonClearedCartonDetailsReceivingBranch[i]['carton_no']+this['tot_non_clr_ctn']  
          ...
          for (let i = 0; i < this.NonClearedCartonDetailsReceivingBranch.length; i++) {
            for (let j = 0; j < this.ClearedCartonDetailsReceivingBranch.length; j++) {
              ...
              if(get_data['data'][i].invoice_no == this.ClearedCartonDetailsReceivingBranch[j].invoice_no)
              {
      
                get_data['data'][i].color = color
                this.ClearedCartonDetailsReceivingBranch[j].color = color
                console.log("1", get_data['data'][i].invoice_no,this.ClearedCartonDetailsReceivingBranch[j].invoice_no)
...
//  this.valForm.get('p_barcodeNumber')?.setValue(this.ManualCartonDetails[0].barcode_no)
 console.log(value)
        $ev.preventDefault();
    ...
    if (this.valForm.valid) {
      ...
      else
      {
        ...
        this.service.InsertReceivingBranch(this.valForm.value).subscribe(res1=>{ 
...
this.Save_spin = '2'
    ...
    delete()
    {
                  ...
                  {
                    this.service.deleteClearedCartonReceivingBranch(this.ClearedCartonDetailsReceivingBranch123.mf_details_id).subscribe(res=>{
                      if (res['code'] == '200') {
                        this.coloredToast('success','Delete Success');
                        this.getClearedCartonDetailsReceivingBranch();
                        this.getNonClearedCartonDetailsReceivingBranch();
    ...
    console.log('valueupdate', value)
              this.service.UpdateReceivingBranch(this.valForm.value).subscribe(res2=>{ 
    ...
    this.coloredToast('success','Successfully Added.');
      ...
      if(this.NonClearedCartonDetailsReceivingBranch){
        this.NonClearedCartonDetailsReceivingBranch.forEach((obj) => {
          if (obj.invoice_no && !includes(this.non_arr, obj.invoice_no)) {
            console.log('res', this.non_arr)
            this.non_arr.push(obj.invoice_no)
          }
       ...
       // console.log(event.target.value)
         ...
         // console.log(event.target.value)
             ...
             this.checkData=true;
              ...
              ClearedCartonDetailsReceivingBranch123:any
              data(ClearedCartonDetailsReceivingBranch:any){
                this.ClearedCartonDetailsReceivingBranch123=ClearedCartonDetailsReceivingBranch;
              }
}